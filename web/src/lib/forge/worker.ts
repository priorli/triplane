...
export function buildDesignStudyPrompt(inputs: DesignStudyInputs): string {
  ...
  return FORGE_PROMPT_PREAMBLE + [
    ...
    "## Output location",
    "",
    "Write to `design/studies/pending/` (the forge runner will rename",
    "`pending/` to a timestamped directory after you finish). Source images",
    "are already in `pending/sources/`.",
    "",
    "## Required outputs — BOTH files must be written",
    "",
    "1. **`design/studies/pending/DESIGN_STUDY.md`** — human-readable report.",
    "   Include the sections the skill requires: Summary, Token deltas (with",
    "   confidence notes on OKLch estimates — ±0.02 L, ±5° h), Schema",
    "   extensions (if references imply tokens the bespoke schema can't",
    "   express), Component gaps, and Accessibility spot-checks.",
    "",
    "2. **`design/studies/pending/design-study-result.json`** — machine-readable",
    "   sidecar consumed by the forge `design-apply` phase. This is NOT",
    "   optional in the forge flow — without it, the whole design pipeline",
    "   silently no-ops and the user's references are ignored. Shape (every",
    "   top-level field optional; emit only what you're confident about,",
    "   but MUST emit the file with at least `confidence`):",
    "",
    "   ```json",
    "   {",
    ...
    "     \"schemaExtensions\": [",
    "       { \"type\": \"accent-color\", \"value\": { \"L\": 0.60, \"C\": 0.15, \"h\": 290 } }",
    "     ],",
    "     \"confidence\": \"high\"",
    "   }",
    "   ```",
    "",
    "   Confidence rules for the forge path:",
    "   - `\"high\"` or `\"medium\"` overall → design-apply will apply every",
    "     populated field and pause for human approval with a preview.",
    "   - `\"low\"` → design-apply refuses to apply (user can override later).",
    "   - Skip the file ENTIRELY only if references provide zero signal on",
    "     brand, fonts, radius, AND accent. If references show purple CTAs",
    "     even at medium confidence, emit the sidecar with brand populated.",
    "",
    "## Run mode",
    "",
    "Write the two files above; do NOT edit `design/tokens.json`,",
    "`design/tokens.schema.json`, `bin/design-tokens.sh`, or any component",
    "file. The forge `design-apply` phase applies the deltas atomically and",
    "pauses for human approval.",
    "",
    "Start now by reading `.claude/skills/design-study/SKILL.md` and the",
    "inputs under `design/studies/pending/sources/`.",
  ].join("\n");
}
